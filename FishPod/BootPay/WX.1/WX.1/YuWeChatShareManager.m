...
#define WXAppSecret @"3c3cf524a92f06ce3acc32ed119c81e5"
#define WX_UL @"https://fish.diaoyuphb.com/"

@interface YuWeChatShareManager()
...
-(void)registWX{
 ...
 BOOL result =   [WXApi registerApp:WXAPPID universalLink:WX_UL];
    if (result ==NO) {
        NSLog(@"微信注册失败,%s",__func__);
    }
}
...
#define WxPayKey @"WxPayKey"
+(void)pay:(NSDictionary *)dict responseView:(nonnull id<YuWXPayResponse>)responseView{
    ...
    req.sign = dict[@"sign"];
    objc_setAssociatedObject([YuWeChatShareManager manager], WxPayKey, responseView, OBJC_ASSOCIATION_ASSIGN);
...
- (void)onResp:(BaseResp *)resp
{
    ...
    if([resp isKindOfClass:[PayResp class]]){
        ...
        id<YuWXPayResponse> wxPayResponse = objc_getAssociatedObject(self, WxPayKey);
        if (wxPayResponse) {
            PayResp *payResponse = ( PayResp *)resp;
            [wxPayResponse wxPayResponseCode:payResponse.errCode
                                      errStr:payResponse.errStr
                                        type:payResponse.type withReturnKey:payResponse.returnKey];
...
#pragma 微信登录
#define WX_HOLDER_VC @"WX_HOLDER_VC"
-(void)loginFromVC:(UIViewController *)holderController{
    //判断微信是否安装
       if([WXApi isWXAppInstalled]){
           ...
           req.state = @"App";
           objc_setAssociatedObject(self, WX_HOLDER_VC,
                                    holderController,
...
-(void)resoveWXLoginResponse:(BaseResp *)resp{
    ...
    id<YuWXResponse> wxResponse = objc_getAssociatedObject(self, WX_HOLDER_VC);
    if ([wxResponse respondsToSelector:@selector(wxLoginFetchCode:withManager:)]) {
        [wxResponse wxLoginFetchCode:code
                         withManager:self];
    }else{
        NSLog(@"%@", [NSString stringWithFormat:@"%@需要实现wxLoginFetchCode:withManager:",wxResponse]);
    }
...
- (void)getWeiXinOpenId:(NSString *)code{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        //         获取微信用户信息
        [self getUserInfoWithAccessToken:access_token WithOpenid:openid withAllInfo: dic];
        
    });
}
-(void)getUserInfoWithAccessToken:(NSString *)access_token WithOpenid:(NSString *)openid withAllInfo:(NSDictionary *) info
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [dic setValue:info[@"refresh_token"] forKey:@"refresh_token"];
            NSLog(@"用户信息字典:===%@",dic);
            //保存改用户信息(我用单例保存)
            NSMutableDictionary * allInfo =[NSMutableDictionary dictionaryWithDictionary:dic];
            ...
            id<YuWXResponse> wxResponse = objc_getAssociatedObject(self, WX_HOLDER_VC);
            if ([wxResponse respondsToSelector:@selector(wxLoginResponse:result:)]) {
//                [wxResponse wxLoginResponse:self result:[YuWxResponseValue modelWithDictionary:allInfo]];
            }
        });
