...
{
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"shengshixian" ofType:@"json"];
    ...
    if (tableView == self.provinceTableView) {
        ...
        for (NSDictionary *dic in self.provinceArray) {
            if ([dic[@"name"] isEqualToString:self.provinceArray[indexPath.row][@"name"]]) {
                ...
                if(_cityArr.count==0)
                {
                    self.citySelectBlock( _selectProvince, nil, nil,self.provinceArray[indexPath.row][@"code"]);
                    [self dismissViewControllerAnimated:YES completion:nil];
                }
...
- (void)didHeadSectionView:(UIButton *)btn
{
    ...
    _areaArr = _cityArr[btn.tag - 100][@"children"];
         ...
         if(_areaArr.count == 0)
         {
             self.citySelectBlock( _selectProvince, _cityArr[btn.tag - 100][@"name"], nil , _cityArr[btn.tag - 100][@"code"]);
             [self dismissViewControllerAnimated:YES completion:nil];
         }
