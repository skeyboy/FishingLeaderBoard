...
+ (instancetype)defaultMonthOpeartion{
    ...
    dispatch_once(&onceToken, ^{
        ...
        obj.selectedDays = [[NSMutableArray alloc]initWithCapacity:0];
    });
    return obj;
}
...
- (void)calendarContainerWithNearByMonths:(NSInteger)currentTag daysOfMonth:(void(^)(NSArray *daysArray,MonthModel *month))calendar{
    ...
    for (int idex = 0;idex<currentModel.totalDays ;idex++) {
        ...
        {
            DayModel *dm = self.selectedDays[i];
        if((dm.year==dayModel.year)&&(dm.month==dayModel.month)&&(dm.day==dayModel.day))
            {
                dayModel.isSelectedDay = YES;
            }
        }
        //早于当前的日期不能选
        dayModel.isInMonth = ![self zaoYuCurrentDayNowMonth:dayModel];
        [daysArray addObject:dayModel];
    }
...
- (BOOL)zaoYuCurrentDayNowMonth:(DayModel*)dayModel{
    NSInteger nowDay = [NSDate dateDay:[NSDate date]];
    NSInteger nowMonth = [NSDate dateMonth:[NSDate date]];
    NSInteger nowYear = [NSDate dateYear:[NSDate date]];
    if (dayModel.year<nowYear) {
    ...
    if ((dayModel.year==nowYear)&&(dayModel.month < nowMonth)) {
        return YES;
    }
    if ((dayModel.year==nowYear)&&(dayModel.month == nowMonth)&&(dayModel.day<nowDay)) {
        return YES;
    }
    return NO;
